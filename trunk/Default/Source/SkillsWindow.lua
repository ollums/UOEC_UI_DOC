...
local tab1 = { 6, 7, 10, 12, 22, 28, 53 }
local tab2 = { 2, 5, 18, 21, 23, 31, 40, 50, 51, 58, 54 }
local tab3 = { 1, 8, 11, 14, 20, 27, 30, 35, 52, 55 }
local tab4 = { 9, 13, 17, 32, 33, 34, 38, 39, 46, 47, 37, 26 }
local tab5 = { 3, 4, 19, 24, 56, 57 }
...
function SkillsWindow.ShowTab(tabnum)
	...
	for i=1,SkillsWindow.NUM_SKILLS_PER_TAB do
		...
		if i <= numItemsInTab then
			...
			skillLevelCap = WindowData.SkillDynamicData[serverId].SkillCap
...
function SkillsWindow.SetStateButton(button, val)
	...
	elseif val == 2 then
		...
		ButtonSetTexture(button, InterfaceCore.ButtonStates.STATE_NORMAL_HIGHLITE, "UO_Core", 92, 338) -- 248
		ButtonSetTexture(button, InterfaceCore.ButtonStates.STATE_PRESSED_HIGHLITE, "UO_Core", 92, 338) -- 248
	end
end
...
function SkillsWindow.OnShown()
    ButtonSetPressedFlag("MenuBarWindowToggleSkills",true)
end
