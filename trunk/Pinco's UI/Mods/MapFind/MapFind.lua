...
function MapFind.Initialize()
	...
	LabelSetText(windowName .. "InfoText", UI.GetCliloc(SystemData.Settings.Language.type, 5024))
...
function MapFind.MagnifyTooltip()
	Tooltips.CreateTextOnlyTooltip(SystemData.ActiveWindow.name,  UI.GetCliloc(SystemData.Settings.Language.type, 5024))
...
function MapFind.SearchText(null, text)
	...
	LabelSetText("MapFindWindowTotal", UI.GetCliloc(SystemData.Settings.Language.type, 5025) .. tabelIdx - 1 )
	ScrollWindowUpdateScrollRect( "MapFindSW" )   	
	local listOffset = ScrollWindowGetOffset("MapFindSW")
	local maxOffset = VerticalScrollbarGetMaxScrollPosition("MapFindSWScrollbar")
	if( listOffset > maxOffset ) then
