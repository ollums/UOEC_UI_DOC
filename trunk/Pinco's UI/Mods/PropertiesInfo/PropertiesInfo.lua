...
function PropertiesInfo.Initialize()
	...
	LabelSetText(windowName .. "InfoText", UI.GetCliloc(SystemData.Settings.Language.type, 989))
...
function PropertiesInfo.MoreInfoOnMouseOver()
	Tooltips.CreateTextOnlyTooltip(SystemData.ActiveWindow.name,  UI.GetCliloc(SystemData.Settings.Language.type, 989))
...
function PropertiesInfo.SearchText(null, text)
	...
	LabelSetText("PropertiesInfoWindowTotal", UI.GetCliloc(SystemData.Settings.Language.type, 991) .. tabelIdx - 1 )
	ScrollWindowUpdateScrollRect( "PropertiesInfoSW" )   	
	local listOffset = ScrollWindowGetOffset("PropertiesInfoSW")
	local maxOffset = VerticalScrollbarGetMaxScrollPosition("PropertiesInfoSWScrollbar")
	if( listOffset > maxOffset ) then
...
function PropertiesInfo.Restart()
	...
	LabelSetText("PropertiesInfoWindowTotal", UI.GetCliloc(SystemData.Settings.Language.type, 990) .. tabelIdx - 1 )
	ScrollWindowUpdateScrollRect( "PropertiesInfoSW" )   	
	local listOffset = ScrollWindowGetOffset("PropertiesInfoSW")
	local maxOffset = VerticalScrollbarGetMaxScrollPosition("PropertiesInfoSWScrollbar")
	if( listOffset > maxOffset ) then
