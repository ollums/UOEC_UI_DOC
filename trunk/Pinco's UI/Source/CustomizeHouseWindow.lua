...
1070642 : south piece
1070643 : west piece
1070644 : east-west intersection
1070645 : north-south intersection
1070646 : north-west intersection
1070647 : north-east intersection
1070648 : south-east intersection
1070649 : south-west intersection
1070650 : northern T-intersection
1070651 : eastern T-intersection
1070652 : southern T-intersection
1070653 : western T-intersection
1070654 : X-intersection
1070655 : surface piece
1070656 : southeast edging
1070657 : northwest edging
1070658 : southwest edging
1070659 : northeast edging
1070660 : north interior piece
1070661 : north edge piece
1070662 : east interior piece
1070663 : east edge piece
1070664 : south interior piece
1070665 : south edge piece
1070666 : southwest interior piece
1070667 : southwest edge piece
1070668 : northeast interior piece
1070669 : northeast edge piece
1070670 : northwest interior piece
1070671 : northwest edge piece
1070672 : southeast interior piece
1070673 : southeast edge piece
1070674 : north-south rib southern end
1070675 : north-south rib piece
1070676 : north-south rib northern end
1070677 : east-west rib western end
1070678 : east-west rib piece
1070679 : east-west rib eastern end
1070680 : west interior piece
1070681 : west edge piece
1070682 : northeastern Y-intersection
1070683 : southeastern Y-intersection
1070684 : western Y-intersection
1070685 : eastern Y-intersection
1070686 : northern Y-intersection
1070687 : southern Y-intersection
1070688 : northwestern Y-intersection
1070689 : southwestern Y-intersection
--]]
...
function CustomizeHouseWindow.getDoorSwingDirectionIcon( categoryID, iconIndex )
    ...
    -- in SA they didn't follow the normal conventions for doors
	elseif categoryID == 20680 or categoryID == 17262 then
		if iconIndex == 1 then
			direction = 1
		elseif iconIndex == 2 then
			direction = 2
	...
	elseif categoryID == 20802 or categoryID == 19746 or categoryID == 18141 or categoryID == 16847 or categoryID == 16834 or categoryID == 16652 or categoryID == 16539 then
		if iconIndex == 1 then
			direction = 2
		elseif iconIndex == 2 then
			direction = 7
...
function CustomizeHouseWindow.Update()
	...
	if numOfComponents >= maxComponents then
		...
		LabelSetTextColor (CustomizeHouseWindow.windowName.."ComponentsUsedText", 255, 255, 255)
	end
	...
	if numOfFixtures >= maxFixtures then
		...
		LabelSetTextColor (CustomizeHouseWindow.windowName.."FixturesUsedText", 255, 255, 255)
	end
...
function CustomizeHouseWindow.insertIcon( tileTable, iconIndex, iconID )
	if iconID ~= 0 and iconID ~= CustomizeHouseWindow.lastIconID then
		table.insert( tileTable, { iconIndex, iconID } ) 
...
function CustomizeHouseWindow.createTileStruct( tileType )
	...
	for categoryIndex in pairs( WindowData.CustomizedHousing.Type[tileType].Category ) do
		...
		local entitlementTID = WindowData.CustomizedHousing.Type[tileType].Category[categoryIndex].entitlementTID
		...
		table.insert( categories.icons, { categoryIndex, categoryTile } )
		CustomizeHouseWindow.entitlements[categoryID] = entitlement
		CustomizeHouseWindow.entitlementTIDs[categoryID] = entitlementTID
	  ...
	  if WindowData.CustomizedHousing.Type[tileType].Category[categoryIndex].Group ~= nil then
		...
		for groupIndex in pairs( WindowData.CustomizedHousing.Type[tileType].Category[categoryIndex].Group ) do	
			...
			for solidIndex in pairs( WindowData.CustomizedHousing.Type[tileType].Category[categoryIndex].Group[groupIndex].Solid ) do
				...
				CustomizeHouseWindow.insertIcon( categories[categoryID].icons, solidIndex, tileID )
			end
			...
			if WindowData.CustomizedHousing.Type[tileType].Category[categoryIndex].Group[groupIndex].Window ~= nil then
				for windowIndex in pairs( WindowData.CustomizedHousing.Type[tileType].Category[categoryIndex].Group[groupIndex].Window ) do	
					...
					CustomizeHouseWindow.insertIcon( categories[categoryID].icons, windowIndex, tileID )
				end
			end
			...
			if WindowData.CustomizedHousing.Type[tileType].Category[categoryIndex].Group[groupIndex].Multi ~= nil then
				for multiIndex in pairs( WindowData.CustomizedHousing.Type[tileType].Category[categoryIndex].Group[groupIndex].Multi ) do	
					...
					CustomizeHouseWindow.insertIcon( categories[categoryID].icons, multiIndex, tileID )
...
function CustomizeHouseWindow.OnIconSelected()
	...
	if ( CustomizeHouseWindow.currentCategoryID ~= nil and CustomizeHouseWindow.entitlements[CustomizeHouseWindow.currentCategoryID] == false ) then
		...
		local entitlementTID = CustomizeHouseWindow.entitlementTIDs[CustomizeHouseWindow.currentCategoryID]
		local windowData = 
		{
			body = WindowUtils.translateMarkup( GetStringFromTid(entitlementTID) ), -- You must upgrade ...
			buttons = { okayButton },
...
function CustomizeHouseWindow.HandleTeleportersButton()
	...
	local categoryID = WindowData.CustomizedHousing.Type[CustomizeHouseWindow.TILE_TYPE_TELEPORTERS].Category[1].previewTile
	Debug.Print("CategoryID: "..tostring(categoryID))
	CustomizeHouseWindow.displayCategory(categoryID)
end
...
function CustomizeHouseWindow.findMaxScale( icons )
	...
	local iconIndex, iconID
	for dataIndex, iconData in pairs( icons ) do
...
function CustomizeHouseWindow.displayIcons( category )
	...
	local shouldDisplayInteriorStairLabels = false
	...
	local iconIndex, iconID
	for dataIndex, iconData in ipairs( category.icons ) do
		iconIndex = iconData[1]
		iconID = iconData[2]
		
		-- Do not show 2nd, 3rd, 4th Exterior Step
		if(CustomizeHouseWindow.tileType == CustomizeHouseWindow.TILE_TYPE_STAIRS and
			CustomizeHouseWindow.currentlyDisplaying == L"Tiles") then

			if( CustomizeHouseWindow.multiID[iconID] == nil and
				iconIndex >= 2 and iconIndex <= 4) then
		...
		if CustomizeHouseWindow.multiID[iconID] then
			...
			if shouldDisplayStairLabels then
				...
				shouldDisplayInteriorStairLabels = true -- shows the interior stairs label
			end
	...
	WindowSetShowing( parentWindow.."InteriorStairsLabel", shouldDisplayInteriorStairLabels )
...
function CustomizeHouseWindow.ItemMouseOver()
--[[
	local iconID = WindowGetId( SystemData.ActiveWindow.name )
	local offset = 0
	if (iconID < 16384) then
		offset = 1020000
	elseif (iconID < 32768) then
		offset = 1095256 - 16384
	elseif (iconID < 65536) then
		offset = 1116792 - 32768
	end
	local tid = iconID + offset
	local name = GetStringFromTid(tid)
	if tid then
		Tooltips.CreateTextOnlyTooltip( SystemData.ActiveWindow.name, name)
		Tooltips.Finalize()
		Tooltips.AnchorTooltip( Tooltips.ANCHOR_WINDOW_TOP )
	end 
--]]
