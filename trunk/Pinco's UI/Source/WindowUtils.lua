...
function WindowUtils.OnLButtonUp( flags, x, y )
	...
	if( WindowUtils.resizing ) then
        ...
        local posX, posY = WindowGetScreenPosition( WindowUtils.resizeWindow  )
...
function WindowUtils.GetStringFromTid(value)
	if (SystemData.Settings.Language.type == SystemData.Settings.Language.LANGUAGE_ENU) then
...
function WindowUtils.SetWindowTitle(window,title)
	...
	title = WindowUtils.Decapitalizing(title)
...
function WindowUtils.RestoreWindowPosition(window, trackSize, alias, ignoreBounds)
	...
	--Debug.Print("WindowUtils.RestoreWindowPosition("..window..")")
	...
	if( index ~= nil ) then
		...
		--local text = string.find(window , "Hotbar")
		if ( WindowIsSticky(window) ) then
			if( x+winWidth > resx ) then
				x = resx - winWidth
			end
			if( y+winHeight > resy ) then
				y = resy - winHeight
			end
		end
...
function WindowUtils.ForceResetWindowPosition(window)
	...
	if( index ~= nil ) then		
		...
		if( x ~= nil and y ~= nil ) then			
			WindowClearAnchors(window)
			WindowAddAnchor(window, "topleft","Root" , "topleft", x, y)				
			if(window == "MenuBarWindow") then
				WindowUtils.CopyScreenPosition("MenuBarWindow", "MenuBarWindowMinimized", 0, 0 )
			end
		end
	end
end
...
function WindowUtils.SaveWindowPosition(window, closing, alias)
	--Debug.Print("WindowUtils.SaveWindowPosition("..window..")")
	if (WindowUtils.blockSave) then
		return
	end
	--if no explicit alias, then use actual window name:
	if not alias then
	...
	if( WindowUtils.openWindows[alias] == true) then
		...
		local text = string.find(window, "Hotbar")
		if (text) then
			local closed = CustomSettings.LoadBoolean( window.. "Closed",false )
			if (closed) then
				width = CustomSettings.LoadNumber( window .. "SizeW", width )
				height = CustomSettings.LoadNumber( window .. "SizeH", width )
			end
		end
...
function WindowUtils.ClearSettings()
	HotbarSystem.DestroyHotbar(EnhancePack.MenuId)
	local windowPositions = SystemData.Settings.Interface.WindowPositions
	windowPositions.Names = nil
	windowPositions.WindowWidth = nil
	windowPositions.WindowHeight = nil
	windowPositions.WindowPosX = nil
	windowPositions.WindowPosY = nil	
	WindowUtils.openWindows = nil
	WindowUtils.trackSize = nil	
end

function WindowUtils.ClearWindowPosition(window)
	local windowPositions = SystemData.Settings.Interface.WindowPositions
	
	local index = CustomSettings.GetSettingIndex( window )
	if( index == nil ) then
		for i, windowName in pairs(windowPositions.Names) do
			if( window == windowName ) then
				index = i
...
function WindowUtils.translateMarkup(str)
	...
	if(str == L"" or str == "" or str == nil) then 
		return L""
	end	
...
local DEBUG = false -- enable for verbose debugging of this function
	...
	if isTitle then
		labelText = WindowUtils.Decapitalizing(labelText)
...
function SnapUtils.StartWindowSnap( windowName )
	if ( not EnhancePack.EnableSnapping) then
...
function SnapUtils.StartLittleWindowSnap( windowName )
	if ( not EnhancePack.EnableSnapping) then
		WindowSetMoving(windowName,true)
		return 
	end
    ...
    WindowUtils.CopySize( windowName, "SnapWindow", 0, 0 )
    WindowSetShowing("SnapWindow",false)
     local width, height = WindowGetDimensions( "SnapWindow" )
    width  = width - 10
    WindowSetDimensions( "SnapWindow", width, height ) 
...
function SnapUtils.FindSnap()
    ...
    for windowName,_ in pairs(SnapUtils.SnappableWindows) do
        ...
        then      
			if string.find(SnapUtils.CurWindow, "Dockspot") or SnapUtils.CurWindow == "PetWindow"  or SnapUtils.CurWindow == "DurabilityBar" or SnapUtils.CurWindow == "MobilesOnScreenWindow" then
				if string.find(windowName, "MobileHealthBar_") then
					continue
				end
			end
    ...
    if( SnapUtils.SnapWindow ) then
		...
		local extrax = 0
        local extray = 0
        local anch = SnapUtils.ANCHOR_POINTS[anchorPt].name
        local anchto = SnapUtils.ANCHOR_POINTS[anchorToPt].name
        
         local text = string.find(SnapUtils.CurWindow , "Hotbar")
         
        
        
		if (EnhancePack.HotbarNoSpace) then
			if ( text ~= nil ) then
				local width, height = WindowGetDimensions(SnapUtils.CurWindow)
				text = string.find(SnapUtils.SnapWindow , "Hotbar")
				if ( text ~= nil ) then
					local width2, height2 = WindowGetDimensions(SnapUtils.SnapWindow)
					if (width >= height) then
						if (width2 >= height2) then
							--Debug.Print("horiz vs horiz")
							if (anchorPt == 3 and anchorToPt ~= 9) then
								extrax = -20
							end
							if (anchorPt == 1 and anchorToPt ~= 7) then
								extrax = 20
							end
						else
							--Debug.Print("horiz vs vert")
							if (anchorPt == 3 or anchorPt == 6 or anchorPt == 9 or anchorPt == 7 or anchorPt == 1) then
								if (anchorToPt ~= 9 and anchorToPt ~= 3) then
									extrax = -20
									if (anchorPt == 1) then
										extray = 20
									end
						...
						if (width2 >= height2) then
							--Debug.Print("vert vs horiz")
							--Debug.Print(anchorPt)
							if (anchorPt == 1 or anchorPt == 3 or anchorPt == 9 or anchorPt == 8 or anchorPt == 7 ) then
								if (anchorToPt ~= 7 and anchorToPt ~= 9) then
									extray = -20
								end
								if (anchorPt == 7 or anchorPt == 1) then
									extrax = 20
								end
							end
						else
							--Debug.Print("vert vs vert")
							if (anchorPt == 7 ) then
								if (anchorToPt ~= 9) then
									extray = -20
								end
							end
							if (anchorPt == 1 ) then
								if (anchorToPt ~= 3) then
									extray = 20
								end
							end
						end
					...
					if (width >= height) then
						--Debug.Print("horiz vs other window")
						if (anchorPt == 9 or anchorPt == 6 or anchorPt == 3 or anchorPt == 1 or anchorPt == 7) then
							if (anchorToPt ~= 9 and anchorToPt ~= 3) then
								extrax = -20
							end
						end
					else
						--Debug.Print("vert vs other window")
						if (anchorPt == 9 or anchorPt == 8 or anchorPt == 7 or anchorPt == 3 or anchorPt == 1) then
							if (anchorToPt ~= 9 and anchorToPt ~= 7) then
								extray = -20
							end
						end
					end
				...
				if ( text ~= nil ) then
					local width2, height2 = WindowGetDimensions(SnapUtils.SnapWindow)
					if (width2 >= height2) then
						--Debug.Print("normal vs horiz")
						--Debug.Print(anchorPt)
						if (anchorPt == 1 or anchorPt == 7 ) then
							extrax = 20
						end
					else
						--Debug.Print("normal vs vert")
						--Debug.Print(anchorPt)
						if (anchorPt == 1 or anchorPt == 2 or anchorPt == 3 ) then
							extray = 20
						end
					end
				end
       ...
       if (SnapUtils.SnapWindow == "StatusWindow") then
			...
			elseif (anchorPt == 7 or anchorPt == 8 or anchorPt == 9) then
				if (anchorPt == 7) then
					anch = "bottom"
					anchto = "top"
				end
        ...
        WindowAddAnchor( "SnapWindow", anch, SnapUtils.SnapWindow, anchto, extrax, extray )
