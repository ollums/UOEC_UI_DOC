...
function EquipmentData.UpdateItemIcon(elementIcon, item)
    if( elementIcon ~= nil and item ~= nil and item.newWidth ~= nil and item.newHeight ~= nil ) then
		...
		local updateId = item.id
		if (EnhancePack and EnhancePack.TrapBoxID == updateId) then				
			hue = 1152
		end
		if (EnhancePack and EnhancePack.LootBoxID == updateId) then				
			hue = 1177
		end
		if EnhancePack and EnhancePack.HighlightLast == updateId and EnhancePack.HighlightLastSetting then
			hue  = 2966
		end
