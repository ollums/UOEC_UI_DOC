...
function TradeWindow.OnAccept()
	...
	local parentWindow = WindowGetParent(nextWindow)
	TradeWindow.PetData = {}
	local data = WindowData.ContainerWindow[TradeWindow.TradeInfo[parentWindow].containerId]
	if (data) then
		
		local numItems = data.numItems
		for i = 1, numItems do
			item = data.ContainedItems[i]
			TradeWindow.PetData[item.objectId] = true
		end
	end
...
function TradeWindow.CloseWindow()
	...
	for key, val in pairs(TradeWindow.PetData) do
		local found = false
		for mob, value in pairs(MobileHealthBar.Handled) do
			MobilesOnScreen.ReversePet[mob] = nil
