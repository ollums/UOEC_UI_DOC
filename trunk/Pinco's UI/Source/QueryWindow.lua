...
function QueryWindow.CheckScannedLegacyMacros()
	if( SystemData.Settings.Interface.scannedLegacyMacros == false and MacroSystemDoLegacyMacrosExist() == true ) then
		...
		ButtonSetPressedFlag( "ScannedLegacyMacrosWindowButtonTwoLabel", true )
...
function QueryWindow.CheckQueriedLegacySettings()
	if( SystemData.Settings.Interface.queriedLegacySettings == false ) then
		...
		LabelSetText( "QueriedLegacySettingsWindowButtonOneLabel", UI.GetCliloc(SystemData.Settings.Language.type, 317) )
...
function QueryWindow.Okay_OnLButtonUp()
	...
	if (SystemData.Settings.Interface.queriedLegacySettingsSelection == 1) then
		SettingsWindow.DefaultRequest = true
		ToggleWindowByName( "SettingsWindow", "", true )
