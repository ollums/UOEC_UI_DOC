...
function PartyHealthBar.CrownMouseOver()
	Tooltips.CreateTextOnlyTooltip(SystemData.ActiveWindow.name, UI.GetCliloc(SystemData.Settings.Language.type, 868))
	Tooltips.Finalize()
...
function PartyHealthBar.CreateHealthBar(mobileId, useDefaultPos)
	...
	if( DoesWindowNameExist(windowName) == false ) then
		...
		if( textureData ~= nil and IsHealthBarEnabled(mobileId)) then
				CircleImageSetTexture("PartyHealthBarPortraitA_"..mobileId,"paperdoll_texture"..mobileId,-textureData.xOffset-11,-textureData.yOffset-191)
		...
		if (StatusWindow.AuraEnabled) then
			...
			if (EnhancePack ~= nil) then
				AnimatedImageSetPlaySpeed ( "PartyGlowingEffect"..mobileId, EnhancePack.AuraSpeed )
			end 
		end
	...
	if(PartyHealthBar.hasWindow[mobileId] == true) then
		...
		if(WindowData.MobileName[mobileId] ~= nil) then
			...
			if (StatusWindow.AuraEnabled) then
				...
				WindowSetTintColor("PartyGlowingEffect"..mobileId,StatusWindow.TextColors[noto].r,StatusWindow.TextColors[noto].g,StatusWindow.TextColors[noto].b)
			else
...
function PartyHealthBar.UpdateHealthBarState(mobileId)
	if(PartyHealthBar.hasWindow[mobileId] == true) then
		...
		local memberIndex = HealthBarManager.GetMemberIndex(mobileId)
		local windowName = "PartyHealthBar_"..memberIndex
		if (EnhancePack ~= nil and StatusWindow.AuraEnabled) then
			AnimatedImageSetPlaySpeed ( "PartyGlowingEffect"..mobileId, EnhancePack.AuraSpeed )
		end 
		-- enable window
...
function PartyHealthBar.RedButton_OnLButtonUp()
	...
	if(mobileId) then
		local spellId = 0
		if (EnhancePack.RedDef == 1) then
			spellId = EnhancePack.RedButton
		elseif (EnhancePack.RedDef == 2) then
			spellId = EnhancePack.RedButton2
		elseif (EnhancePack.RedDef == 3) then
			spellId = EnhancePack.RedButton3
		end
...
function PartyHealthBar.RedButton_OnMouseOver()
	...
	local itemData = { windowName = SystemData.ActiveWindow.name,
							...
							body = UI.GetCliloc(SystemData.Settings.Language.type, 720)}
...
function PartyHealthBar.GreenButton_OnLButtonUp()
	...
	if(mobileId) then
		local spellId = 0
		if (EnhancePack.GreenDef == 1) then
			spellId = EnhancePack.GreenButton
		elseif (EnhancePack.GreenDef == 2) then
			spellId = EnhancePack.GreenButton2
		elseif (EnhancePack.GreenDef == 3) then
			spellId = EnhancePack.GreenButton3
		end
...
function PartyHealthBar.BlueButton_OnLButtonUp()
	...
	if(mobileId) then
		local spellId = 0
		if (EnhancePack.BlueDef == 1) then
			spellId = EnhancePack.BlueButton
		elseif (EnhancePack.BlueDef == 2) then
			spellId = EnhancePack.BlueButton2
		elseif (EnhancePack.BlueDef == 3) then
			spellId = EnhancePack.BlueButton3
		end
...
function PartyHealthBar.Button_OnRButtonUp()
	if (string.find(SystemData.ActiveWindow.name, "Red")) then
		local spellId = EnhancePack.RedButton
		local icon, serverId, tid, desctid = GetAbilityData(spellId)
		if (not tid) then
			tid = 1011051
		end
		ContextMenu.CreateLuaContextMenuItemWithString(GetStringFromTid(tid),0,"r1",2,EnhancePack.RedDef ==1)
		...
		ContextMenu.CreateLuaContextMenuItemWithString(GetStringFromTid(tid),0,"r2",2,EnhancePack.RedDef ==2)
		...
		ContextMenu.CreateLuaContextMenuItemWithString(GetStringFromTid(tid),0,"r3",2,EnhancePack.RedDef ==3)
	elseif (string.find(SystemData.ActiveWindow.name, "Green")) then
		local spellId = EnhancePack.GreenButton
		local icon, serverId, tid, desctid = GetAbilityData(spellId)
		if (not tid) then
			tid = 1011051
		end
		ContextMenu.CreateLuaContextMenuItemWithString(GetStringFromTid(tid),0,"g1",2,EnhancePack.GreenDef ==1)
		...
		ContextMenu.CreateLuaContextMenuItemWithString(GetStringFromTid(tid),0,"g2",2,EnhancePack.GreenDef ==2)
		...
		ContextMenu.CreateLuaContextMenuItemWithString(GetStringFromTid(tid),0,"g3",2,EnhancePack.GreenDef ==3)
	elseif (string.find(SystemData.ActiveWindow.name, "Blue")) then
		local spellId = EnhancePack.BlueButton
		local icon, serverId, tid, desctid = GetAbilityData(spellId)
		if (not tid) then
			tid = 1011051
		end
		ContextMenu.CreateLuaContextMenuItemWithString(GetStringFromTid(tid),0,"b1",2,EnhancePack.BlueDef ==1)
		...
		ContextMenu.CreateLuaContextMenuItemWithString(GetStringFromTid(tid),0,"b2",2,EnhancePack.BlueDef ==2)
		...
		ContextMenu.CreateLuaContextMenuItemWithString(GetStringFromTid(tid),0,"b3",2,EnhancePack.BlueDef ==3)
	end
...
function PartyHealthBar.ContextMenuCallback(returnCode, param)
	if (string.find(returnCode, "r")) then
		...
		EnhancePack.RedDef = tonumber(val )
		CustomSettings.SaveNumber( "EnhancePackRedDef", EnhancePack.RedDef )
	elseif (string.find(returnCode, "g")) then
		local val = string.gsub(returnCode,"g", "")
		EnhancePack.GreenDef = tonumber(val )
		CustomSettings.SaveNumber( "EnhancePackGreenDef", EnhancePack.GreenDef )
	elseif (string.find(returnCode, "b")) then
		local val = string.gsub(returnCode,"b", "")
		EnhancePack.BlueDef = tonumber(val )
		CustomSettings.SaveNumber( "EnhancePackBlueDef", EnhancePack.BlueDef )
	end
end