...
function UO_StandardDialog.CreateDialog(dialogData)
    ...
    WindowClearAnchors(dialogWindowName)
	WindowAddAnchor(dialogWindowName, "center", "Root", "center", 0, 0)
...
function UO_StandardDialog.Initialize()
    ...
    if( dialogData.titleTid ~= nil ) then
        title = EnhancePack.GetStringFromTid(dialogData.titleTid)
    else
    ...
    if (dialogData.body == L"Connection lost") then
		dialogData.bodyTid = 3000004
	end
    if( dialogData.bodyTid ~= nil ) then
        body = EnhancePack.GetStringFromTid(dialogData.bodyTid)
    else
    ...
    for index, button in pairs(dialogData.buttons) do
        ...
        if( button.textTid ~= nil ) then
            text = EnhancePack.GetStringFromTid(button.textTid)
        else
            text = button.text
        end
