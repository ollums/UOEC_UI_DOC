...
function Shopkeeper.Initialize()
	...
	if( WindowData.ShopData.IsSelling == true ) then   
		...
		LabelSetText ("StoreName", wstring.upper(ReplaceTokens(UI.GetCliloc(SystemData.Settings.Language.type, 7000), {name})) )--GetStringFromTid(Shopkeeper.TID.NPCSTORE) )
...
function Shopkeeper.SellTooltip()
	local name = L"Sell " .. Organizer.ActiveSell
	if (Organizer.Restocks_Desc[Organizer.ActiveSell] ~= L"") then
		name = Organizer.Restocks_Desc[Organizer.ActiveSell]
	end
	Tooltips.CreateTextOnlyTooltip(SystemData.ActiveWindow.name,  UI.GetCliloc(SystemData.Settings.Language.type, 610) .. L"\n".. UI.GetCliloc(SystemData.Settings.Language.type, 851) .. name)
...
function Shopkeeper.OnBuyOne()
	...
	if (Shopkeeper[merchantId].Items[itemIndex].totalQuantity > 1) then
		...
		local rdata = {title=GetStringFromTid(1077826), subtitle=UI.GetCliloc(SystemData.Settings.Language.type, 735), callfunction=Shopkeeper.BuyHowMuch, id=Shopkeeper[merchantId].Items[itemIndex].totalQuantity}
