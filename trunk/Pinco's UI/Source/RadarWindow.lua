...
function RadarWindow.Lock()
	...
	CustomSettings.SaveBoolean( "RadarWindowLocked", RadarWindow.Locked  )
end
...
function RadarWindow.UpdateRadar() 
    if( MapCommon.ActiveView == MapCommon.RADAR_MODE_NAME ) then 
        ...
        if (EnhancePack and EnhancePack.EnableCartographer) then
...
function RadarWindow.BigOnLButtonUp()
	...
	CustomSettings.SaveNumber( "RadarWindowScale", RadarWindow.Scale )
	RadarWindow.UpdateRadar() 
end
...
function RadarWindow.RotateButtonMouseOver()
	Tooltips.CreateTextOnlyTooltip(SystemData.ActiveWindow.name, L"Tilt Map")
	Tooltips.Finalize()
	Tooltips.AnchorTooltip( Tooltips.ANCHOR_WINDOW_BOTTOM )
end
