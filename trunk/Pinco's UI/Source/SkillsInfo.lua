...
function SkillsInfo.UpdateGump(DisplaySkill )
	...
	local train = LoadTextFile("./UserInterface/"..SystemData.Settings.Interface.customUiName.."/SkillTraining/skill" .. index .. ".txt")
	local sat = LoadTextFile("./UserInterface/"..SystemData.Settings.Interface.customUiName.."/SkillTraining/satyr.txt")
	local gol = LoadTextFile("./UserInterface/"..SystemData.Settings.Interface.customUiName.."/SkillTraining/golem.txt")
	...
	if ttle then
		...
		LabelSetText( this .. "TtleLbLTitle", UI.GetCliloc(SystemData.Settings.Language.type, 922) )
	...
	if train then
		local tl = lines(WStringToString(train))
		train = ""
		local Satyr = false
		local golem = false
		for i = 1, table.getn(tl) do
			if tl[i] == "|SATYR|" then
				Satyr = true
			elseif tl[i] == "|GOLEM|" then
				golem = true
			else
				if i == 1 then
					train = tl[i]
		...
		LabelSetText( this .. "TrainLbLTitle", UI.GetCliloc(SystemData.Settings.Language.type, 918)  )
		...
		if Satyr then
			-- "Satyr Trick" (Optional)
			if sat then
				local tl = lines(WStringToString(sat))
				sat = ""
				for i = 1, table.getn(tl) do
					if i == 1 then
						sat = tl[i]
				...
				sat = StringToWString(sat)
				
				CreateWindowFromTemplate(  this .. "SatLbLTitle", "LabelTitle", "SkillInfoTextWindowScrollChildText" )
				WindowAddAnchor( this .. "SatLbLTitle", "bottomleft", lastLBL, "topleft", 0, 10 )
				LabelSetText( this .. "SatLbLTitle", UI.GetCliloc(SystemData.Settings.Language.type, 919)  )
				
				local x, y = WindowGetDimensions( this .. "SatLbLTitle")
				totalH = totalH + y + 10
				
				CreateWindowFromTemplate(  this .. "SatLbLDescription", "LabelDescription", "SkillInfoTextWindowScrollChildText" )
				WindowAddAnchor( this .. "SatLbLDescription", "bottomleft",  this .. "SatLbLTitle", "topleft", 0, 8 )
				LabelSetText( this .. "SatLbLDescription", sat )
				lastLBL =  this .. "SatLbLDescription"
				
				local x, y = WindowGetDimensions( this .. "SatLbLDescription")
				totalH = totalH + y + 8
			end
		end
		
		if golem then
			-- "Golem Trick" (Optional)
			if gol then
				local tl = lines(WStringToString(gol))
				sat = ""
				for i = 1, table.getn(tl) do
					if i == 1 then
						gol = tl[i]
				...
				gol = StringToWString(gol)
				
				CreateWindowFromTemplate(  this .. "GolLbLTitle", "LabelTitle", "SkillInfoTextWindowScrollChildText" )
				WindowAddAnchor( this .. "GolLbLTitle", "bottomleft", lastLBL, "topleft", 0, 10 )
				LabelSetText( this .. "GolLbLTitle", UI.GetCliloc(SystemData.Settings.Language.type, 920)  )
				
				local x, y = WindowGetDimensions( this .. "GolLbLTitle")
				totalH = totalH + y + 10
				
				CreateWindowFromTemplate(  this .. "GolLbLDescription", "LabelDescription", "SkillInfoTextWindowScrollChildText" )
				WindowAddAnchor( this .. "GolLbLDescription", "bottomleft",  this .. "GolLbLTitle", "topleft", 0, 8 )
				LabelSetText( this .. "GolLbLDescription", gol )
				lastLBL =  this .. "GolLbLDescription"
				
				local x, y = WindowGetDimensions( this .. "GolLbLDescription")
				totalH = totalH + y + 8
			end
		end
	end
