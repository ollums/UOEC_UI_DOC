...
function OverheadText.CountNamesData()
	local a = 0
...
function OverheadText.UpdateName(mobileId)
	...
	if( DoesWindowNameExist( windowName) == false or data == nil ) then
		return
	end
	local sfind = wstring.find
	if(data.MobName ~= nil ) then
		...
		if (mobileId ~= WindowData.PlayerStatus.PlayerId and EnhancePack.ToggleYouSee and not OverheadText.LastSeeName[mobileId]) then
			...
			if (data.MobName ~= nil and data.MobName ~= L"" and data.Notoriety+1 ~= NameColor.Notoriety.NONE and passa) then	
				if (not OverheadText.LastSeeName[mobileId]) then
					if (EnhancePack.UseNewChat) then
						local logVal = {text = GetStringFromTid(3002000) ..data.MobName, channel = 15, color = NameColor.TextColors[data.Notoriety+1], sourceId = mobileId, sourceName = data.MobName, ignore = false, category = 0, timeStamp = StringToWString(string.format("%02.f", EnhancePack.Clock.h) .. ":" .. string.format("%02.f", EnhancePack.Clock.m) .. ":" .. string.format("%02.f", EnhancePack.Clock.s)), Notoriety = data.Notoriety }
		...
		local summonNameFix = string.gsub(WStringToString(data.MobName), "^%s*(.-)%s*$","%1")
		summonNameFix = string.lower(summonNameFix)
		if (CreaturesDB.IsPixie(summonNameFix)) then
			summonNameFix="fey"
		end

		if (CreaturesDB.IsDaemon(summonNameFix)) then

			summonNameFix="daemon"
		end
		if (MobilesOnScreen.IsPet(mobileId) and CreaturesDB.SummonTimes[summonNameFix] and MobileHealthBar.SummonTimer[mobileId] == nil) then
			MobileHealthBar.SummonTimer[mobileId] = EnhancePack.TimeSinceLogin + CreaturesDB.SummonTimes[summonNameFix]
		end
...
function OverheadText.UpdateOverheadAnchors(mobileId)
	...
	WindowSetHandleInput(overheadNameWindow, EnhancePack.clickableNames)
	WindowSetScale(windowName, EnhancePack.OverhedTextSize)
...
function OverheadText.NameOnDblClick()
	...
	if ( data ~= nil ) then
		...
		if prop and (wstring.find(wstring.lower(prop), wstring.lower(tid)) or prop == qg)  then
...
function OverheadText.ShowOverheadText()
	...
	if ( SystemData.Text == EnhancePack.LastMessage ) then
	...
	if ( find(senderText , " looks ill.*") 
		...
		) and not EnhancePack.noPoisonOthers then
		return
	end
	
	local spell = SpellsInfo.SpellsData[WStringToString(SystemData.Text)]
	local IsSpell = spell ~= nil
	if (IsSpell and EnhancePack.DisableSpells) then
		SystemData.Text = L""
		return
	end
	
	if (IsSpell and EnhancePack.ShowSpellName ) then
	...
	if (FontSelector.Fonts[OverheadText.SpellsFontIndex].fontName == "runic" and EnhancePack.ShowSpellName) then
		OverheadText.SpellsFontIndex = 1
	end
	...
	if( OverheadText.ChatData[SystemData.TextSourceID].numVisibleChat > 0 ) then
		for i=OverheadText.ChatData[SystemData.TextSourceID].numVisibleChat+1, 2, -1 do 
			if( i <= 3 ) then
				...
				local fontName = LabelGetFont(oldWindow)
				...
				WindowSetScale(newWindow, EnhancePack.OverhedTextSize)
				WindowSetScale(oldWindow, EnhancePack.OverhedTextSize)
	...
	WindowSetHandleInput(overheadChatWindow, EnhancePack.clickableNames)
