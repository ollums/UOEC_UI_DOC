...
ActionEditWindow.UnEquip.Slots[1] = { slot = EquipmentData.EQPOS_HEAD, x = 0, y = 0, texture = "paperdoll_slots" }
ActionEditWindow.UnEquip.Slots[2] = { slot = EquipmentData.EQPOS_NECK, x = 0, y = 58, texture = "paperdoll_slots"}
ActionEditWindow.UnEquip.Slots[3] = { slot = EquipmentData.EQPOS_ARMS, x = 0, y = 116, texture = "paperdoll_slots"}
ActionEditWindow.UnEquip.Slots[4] = { slot = EquipmentData.EQPOS_RIGHTHAND, x = 58, y = 0, texture = "paperdoll_slots"}
ActionEditWindow.UnEquip.Slots[5] = { slot = EquipmentData.EQPOS_FINGER1, x = 58, y = 58, texture = "paperdoll_slots"}
ActionEditWindow.UnEquip.Slots[6] = { slot = EquipmentData.EQPOS_LEGS, x = 58, y = 116, texture = "paperdoll_slots"}
ActionEditWindow.UnEquip.Slots[7] = { slot = EquipmentData.EQPOS_EARS, x = 116, y = 0, texture = "paperdoll_slots"}
ActionEditWindow.UnEquip.Slots[8] = { slot = EquipmentData.EQPOS_CHEST,  x = 116, y = 58, texture = "paperdoll_slots"}
ActionEditWindow.UnEquip.Slots[9] = { slot = EquipmentData.EQPOS_LWRIST, x = 116, y = 116, texture = "paperdoll_slots"}
ActionEditWindow.UnEquip.Slots[10] = { slot = EquipmentData.EQPOS_LEFTHAND, x = 174, y = 0, texture = "paperdoll_slots"}
ActionEditWindow.UnEquip.Slots[11] = { slot = EquipmentData.EQPOS_HANDS, x = 174, y = 58, texture = "paperdoll_slots" }
ActionEditWindow.UnEquip.Slots[12] = { slot = EquipmentData.EQPOS_TALISMAN, x = 174, y = 116, texture = "paperdoll_slots" }
ActionEditWindow.UnEquip.Slots[13] = { slot = EquipmentData.EQPOS_FEET, x = 0, y = 0, texture = "paperdoll_slots2" }
ActionEditWindow.UnEquip.Slots[14] = { slot = EquipmentData.EQPOS_TORSO, x = 58, y = 0, texture = "paperdoll_slots2"  }
ActionEditWindow.UnEquip.Slots[15] = { slot = EquipmentData.EQPOS_WAIST, x = 116, y = 0 , texture = "paperdoll_slots2" }
ActionEditWindow.UnEquip.Slots[16] = { slot = EquipmentData.EQPOS_ABOVECHEST, x = 174, y = 0, texture = "paperdoll_slots2"  }
ActionEditWindow.UnEquip.Slots[17] = { slot = EquipmentData.EQPOS_CAPE, x = 0 , y = 58, texture = "paperdoll_slots2"  }
ActionEditWindow.UnEquip.Slots[18] = { slot = EquipmentData.EQPOS_DRESS, x = 58, y = 58, texture = "paperdoll_slots2"  }
ActionEditWindow.UnEquip.Slots[19] = { slot = EquipmentData.EQPOS_SKIRT, x = 116, y = 58, texture = "paperdoll_slots2"  }
...
function ActionEditWindow.Initialize()
	...
	if( windowName == "ActionEditEquip" ) then
		...
		ButtonSetText(windowName.."ClearButton",UI.GetCliloc(SystemData.Settings.Language.type, 442))
	end
...
function ActionEditWindow.UpdateTextEdit(editWindow, hotbarId, itemIndex, subIndex, actionId)
	...
	if (actionId == 5048 and wstring.len(speechText) > 0) then
		speechText = wstring.gsub(speechText, L"script PlaySound%(1, \"", L"")
		speechText = wstring.gsub(speechText, L"\", 0%)", L"")
		speechText = wstring.gsub(speechText, L"/", L"\\")
	elseif (actionId == 5058 and wstring.len(speechText) > 0) then
		speechText = wstring.gsub(speechText,  L"script EnhancePack.SendOverheadText%(L\"", L"")
		speechText = wstring.gsub(speechText,  L"\", 1152, true, false%)", L"")
	elseif (actionId == 5059 and wstring.len(speechText) > 0) then
		speechText = wstring.gsub(speechText,  L"script SlayerNote.SetNote%(L\"", L"")
		speechText = wstring.gsub(speechText,  L"\"%)", L"")
	elseif (actionId == 5059 and wstring.len(speechText) > 0) then
		speechText = wstring.gsub(speechText,  L"script SlayerNote.SetNote%(L\"", L"")
		speechText = wstring.gsub(speechText,  L"\"%)", L"")
	...
	elseif ( (actionId > 15000 and actionId < 15100) ) then
		if (wstring.len(speechText) > 0) then
			local id = actionId - 15000
			speechText = wstring.gsub(speechText,  L"script EnhancePack.CustomBuff%(" .. id .. L", L\"", L"")
			speechText = wstring.gsub(speechText,  L"%)", L"")
			speechText = wstring.gsub(speechText,  L"\" \, ", L"|")
		else
			speechText = L"Name|0.0"
		end
	end
...
function ActionEditWindow.OpenEditWindow(actionType,anchorWindow,hotbarId,itemIndex,subIndex,actionId)
	...
	if actionId then
		if (actionId==5048 or actionId == 5058 or actionId ==  5059 or (actionId > 15000 and actionId < 15100)) then
	...
	if( actionData ~= nil and actionData.editWindow ~= nil ) then
		...
		--Debug.Print( "ActionEditWindow.OpenEditWindow HotBarId: "..hotbarId.." itemIndex: "..itemIndex.." subIndex: "..subIndex)
		-- set up edit window
...
function ActionEditWindow.TextEditOK()
	...
	local actionId = ActionEditWindow.CurActionId
	local speechText = ActionEditTextEntry.Text
	if ((actionId == 5048 or actionId == 5049) and wstring.len(speechText) > 0) then
		speechText = wstring.gsub(speechText, L"\\", L"/")
		speechText = L"script PlaySound(1, \"" .. speechText .. L"\", 0)"
		
	elseif (actionId == 5058 and wstring.len(speechText) > 0) then
		speechText = L"script EnhancePack.SendOverheadText(L\"" .. speechText .. L"\", 1152, true, false)"
		
	elseif (actionId == 5059 and wstring.len(speechText) > 0) then
		speechText = L"script SlayerNote.SetNote(L\"" .. speechText .. L"\")"
	elseif ( (actionId > 15000 and actionId < 15100)  and wstring.len(speechText) > 0 ) then
		speechText = wstring.gsub(speechText, L"|", L"\" \, ")
		local id = actionId - 15000
		speechText = L"script EnhancePack.CustomBuff(" .. id .. L", L\"" .. speechText .. L")"
	end
